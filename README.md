...
- [Architecture](#architecture)
  - [Vertical delay: BRAM as a shift register](#vertical-delay-bram-as-a-shift-register)
  - [Horizontal delay: per-row shift registers](#horizontal-delay-per-row-shift-registers)
  - [Window assembly and indexing](#window-assembly-and-indexing)
  - [Pipeline warm-up (`window_valid`)](#pipeline-warm-up-window_valid)
  - [Row/frame resynchronization](#rowframe-resynchronization)
- [Interface](#interface)
...
**Stall handling:** `window_valid` must also be explicitly forced low whenever the input stream stalls (`pixel_valid = '0'`, e.g. during a DMA bus pause). Without an explicit `else` branch driving it low in that case, a VHDL signal holds its last value — meaning a stall could otherwise leave `window_valid` stuck high while the window itself is frozen and stale, causing the downstream stage to silently reprocess the same window repeatedly. This is a real bug this design hit in practice (see [Lessons Learned](#lessons-learned--common-pitfalls)).
